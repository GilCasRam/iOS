...
final class HomeViewController: UIViewController {
    ...
    private var operation: OperationsType = .none // Opracion actual
    
    // MARK: - Constantes
    
    private let kDecimaSeparator = Locale.current.decimalSeparator!
    ...
    override func viewDidLoad() {
       ...
       numberDecimal.setTitle(kDecimaSeparator, for: .normal)
    ...
    @IBAction func numberDecimalAction(_ sender: UIButton) {
        ...
        //no estamos operando y hacemos una ceunta para ver si no sobrepasamos los digitos permitidos
        if !operating && currentTemp.count >= kMaxLength{
            return
        }
        resultLabel.text = resultLabel.text! + kDecimaSeparator
        decimal = true
    ...
    @IBAction func numberAction(_ sender: UIButton) {
        ...
        if operating {
            // si total es igual a 0 entonces nuestro total sera el temporal en caso contrario sera el total
        ...
        if decimal {
            //si teniamos un 1 en pantalla ahora sera 1.
            currentTemp = "\(currentTemp)\(kDecimaSeparator)"
            decimal = false
        }
    ...
    private func result() {
        ...
        //Realizar guardados de datos simples en iOS. cada que entremos a result se guardara el resultado
        UserDefaults.standard.set(total, forKey: kTotal)
        
        print("TOTAL: \(total)")
    }

    //Uestra de forma visual la operacion seleccionada
